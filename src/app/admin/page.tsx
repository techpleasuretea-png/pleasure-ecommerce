                ...
                {stats.recentOrders && stats.recentOrders.length > 0 ? (
                    <>
                        ...
                        <div className="md:hidden divide-y divide-gray-100 dark:divide-gray-800">
                            {stats.recentOrders.map((order: any) => (
                                <Link href={`/admin/orders/${order.id}`} key={order.id} className="block p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            <span className="font-mono font-bold text-sm text-primary">#{order.order_number || order.id.slice(0, 8)}</span>
                                            <p className="text-sm font-bold text-text-light dark:text-text-dark mt-0.5">{order.recipient_name || order.profiles?.full_name || "Guest"}</p>
                                        </div>
                                        <span className={`inline-flex px-2 py-1 rounded text-xs font-bold capitalize ${order.status === 'delivered' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                                            order.status === 'shipping' || order.status === 'shipped' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
                                                order.status === 'cancelled' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                                    ...
                                    <div className="flex justify-between items-center text-xs text-subtext-light dark:text-subtext-dark">
                                        ...
                                        <span className="font-mono font-bold text-sm text-text-light dark:text-text-dark">৳{order.total_amount.toFixed(2)}</span>
                                    </div>
                                </Link>
                            ))}
                        </div>
